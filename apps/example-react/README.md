...
3. **State UI**: The hook subscribes to `session.state$` and `session.isConnected$`. `App.tsx` uses `state.status` with `SerialSessionStatus` for status text and `isConnected` for button enablement.
