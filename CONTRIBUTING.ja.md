...
手書き Guide の source と生成される API Reference の出力は分離されています。canonical な構成は [ドキュメント構成](packages/web-serial-rxjs/docs/ARCHITECTURE.ja.md) を参照してください。

| 対象 | 場所 | 編集 |
| --- | --- | --- |
| 日本語 / English Guide（source） | `packages/web-serial-rxjs/docs/guide/{ja,en}/` | 可 |
| API Reference（TypeDoc 出力） | `docs/api/`（#457 以降） | 不可 — CI で生成 |
| デプロイ artifact ルート | `docs/` | 不可 — 生成 HTML はコミットしない |
