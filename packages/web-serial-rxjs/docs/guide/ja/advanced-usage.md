...
**通常は** 組み込みの **`lines$`** で十分です（改行区切りの1行が都度1件 emit）。**`receive$`** は従来どおり、デコーダが返す **未フレーミングの UTF-8 チャンク**（デコード済みテキスト。ワイヤ上の生バイトではない）をそのまま流します。組み込み `lines$` とは異なる区切り文字や正規化が必要なときだけ `scan` などでフレーミングします。
